...
func (s *LinkService) Create(ctx context.Context, input CreateInput) (*model.Link, error) {
	...
	if input.ExpiresIn < 0 {
		return nil, fmt.Errorf("%w: expires_in_seconds cannot be negative", ErrInvalidInput)
	}
	...
	if input.CustomAlias != "" {
		if !aliasPattern.MatchString(input.CustomAlias) {
			return nil, fmt.Errorf("%w: custom_alias must be 4-32 letters, numbers, underscores, or hyphens", ErrInvalidInput)
		}
		if isReservedCode(input.CustomAlias) {
			return nil, fmt.Errorf("%w: custom_alias is reserved", ErrInvalidInput)
		}
...
func (s *LinkService) Resolve(ctx context.Context, code string) (*model.Link, error) {
	...
	link, err := s.repository.Resolve(ctx, code)
	if errors.Is(err, repository.ErrNotFound) {
		existing, getErr := s.repository.GetByCode(ctx, code)
		if getErr == nil && existing.IsExpired(s.now().UTC()) {
			return nil, ErrExpired
		}
	}
	return link, err
...
func validateURL(raw string) (string, error) {
	...
	if len(raw) == 0 || len(raw) > 2048 {
		return "", fmt.Errorf("%w: url must be between 1 and 2048 characters", ErrInvalidInput)
	}
...
func validateCode(code string) error {
	if !aliasPattern.MatchString(code) {
		return fmt.Errorf("%w: invalid short code", ErrInvalidInput)
	}
	return nil
}

func isReservedCode(code string) bool {
	_, reserved := reservedCodes[strings.ToLower(code)]
	return reserved
}
